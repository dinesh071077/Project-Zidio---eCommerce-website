// import React, { useState } from 'react';
// import { useNavigate } from 'react-router-dom';
// import { motion } from 'framer-motion';

// const tshirts = [
//   {
//     id: 1,
//     name: 'Mens Brick Red Unique Graphic Printed Oversized T-shir',
//     price: 549,
//     img: 'https://ik.imagekit.io/zifllo6u3/shopping.webp?updatedAt=1745507297596',
//     description: 'Premium cotton Half Sleeve Oversized Fit Round Neck Casual Wear T-shirt  .',
//   },
//   {
//     id: 2,
//     name: 'Solid Men Oversized T-Shirt - Lilac ',
//     price: 659,
//     img: 'https://ik.imagekit.io/zifllo6u3/download%20(7).jpeg?updatedAt=1745507946106',
//     description: '! ',
//   },
//   {
//     id: 3,
//     name: 'plain oversized unisex   T-shirt',
//     price: 499,
//     img: 'https://ik.imagekit.io/zifllo6u3/2_b773fcb3-6d20-4db5-9d71-8127427cb1aa.webp?updatedAt=1745508058883',
//     description: 'Swing into style with this t-shirt.',
//   },
//   {
//     id: 4,
//     name: 'Cat Design Unisex Terry Oversized T-Shirt',
//     price: 499,
//     img: 'https://ik.imagekit.io/zifllo6u3/TDD_Mockups_c10e2e70-3d9f-489b-849f-b7534d991d75.webp?updatedAt=1745508251237',
//     description: 'Swing into style with this t-shirt.',
//   },
// ];

// function  Oversized() {
//   const navigate = useNavigate();
//   const [loadingId, setLoadingId] = useState(null); // Track which card is loading

//   const viewDetails = (tshirt) => {
//     setLoadingId(tshirt.id);
//     setTimeout(() => {
//       navigate(`/tshirt/${tshirt.id}`, { state: tshirt });
//     }, 1000); // 1 second buffering delay
//   };

//   return (
//     <div className="bg-gray-900 text-white pt-4 px-4 min-h-screen">
//       <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-4 max-w-7xl mx-auto">
//         {tshirts.map((shirt) => (
//           <motion.div
//             key={shirt.id}
//             className={`bg-gray-800 p-4 rounded-xl shadow-md cursor-pointer relative`}
//             onClick={() => viewDetails(shirt)}
//             whileHover={{ scale: 1.05 }}
//             transition={{ duration: 0.3 }}
//           >
//             {loadingId === shirt.id ? (
//               <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-800 bg-opacity-80 rounded-xl">
//                 <div className="w-8 h-8 border-4 border-blue-500 border-t-transparent rounded-full animate-spin mb-2"></div>
//                 <span className="text-sm text-blue-400">Loading...</span>
//               </div>
//             ) : (
//               <>
//                 <img
//                   src={shirt.img}
//                   alt={shirt.name}
//                   className="w-full h-64 sm:h-72 md:h-80 lg:h-96 object-cover rounded-lg mb-4"
//                 />
//                 <h2 className="text-lg font-semibold">{shirt.name}</h2>
//                 <p className="text-blue-400 text-md">₹{shirt.price}</p>
//               </>
//             )}
//           </motion.div>
//         ))}
//       </div>
//     </div>
//   );
// }

// export default Oversized;


import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { motion } from 'framer-motion';

function Oversized() {
  const navigate = useNavigate();
  const [tshirts, setTshirts] = useState([]);
  const [loadingId, setLoadingId] = useState(null); // For buffering on card click
  const [isLoading, setIsLoading] = useState(true); // For initial page loading

  useEffect(() => {
    const fetchTshirts = async () => {
      try {
        const res = await fetch('http://localhost:5000/api/tshirt/type/Over Sized');
        const data = await res.json();
        setTshirts(data);
        setIsLoading(false);
      } catch (err) {
        console.error('Error fetching oversized t-shirts:', err);
      }
    };

    fetchTshirts();
  }, []);

  const viewDetails = (tshirt) => {
    setLoadingId(tshirt._id);
    setTimeout(() => {
      navigate(`/tshirt/${tshirt._id}`, { state: tshirt });
    }, 1000);
  };

  return (
    <div className="bg-gray-900 text-white pt-4 px-4 min-h-screen">
      <h2 className="text-2xl font-bold mb-6">Oversized T-Shirts</h2>
      {isLoading ? (
        <div className="flex justify-center items-center h-64">
          <div className="w-10 h-10 border-4 border-blue-500 border-t-transparent rounded-full animate-spin"></div>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-4 max-w-7xl mx-auto">
          {tshirts.map((shirt) => (
            <motion.div
              key={shirt._id}
              className="bg-gray-800 p-4 rounded-xl shadow-md cursor-pointer relative"
              onClick={() => viewDetails(shirt)}
              whileHover={{ scale: 1.05 }}
              transition={{ duration: 0.3 }}
            >
              {loadingId === shirt._id ? (
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-800 bg-opacity-80 rounded-xl">
                  <div className="w-8 h-8 border-4 border-blue-500 border-t-transparent rounded-full animate-spin mb-2"></div>
                  <span className="text-sm text-blue-400">Loading...</span>
                </div>
              ) : (
                <>
                  <img
                    src={shirt.imageUrl}
                    alt={shirt.name}
                    className="w-full h-64 sm:h-72 md:h-80 lg:h-96 object-cover rounded-lg mb-4"
                  />
                  <h2 className="text-lg font-semibold">{shirt.name}</h2>
                  <p className="text-blue-400 text-md">₹{shirt.price}</p>
                </>
              )}
            </motion.div>
          ))}
        </div>
      )}
    </div>
  );
}

export default Oversized;
